...
Still needs testing.
<pre>directions = GoogleDirections.new(origin, waypoint1, waypoint2, waypoint3, boolean)</pre>
where _origin_ and _waypoint*_ are strings of addresses or places that Google can find an address for.  Example: "816 Meridian St., 37207" and boolean is true or false (if true the route will be optimized).
