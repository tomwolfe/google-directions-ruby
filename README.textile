...
Still needs testing.
<pre>directions = GoogleDirections.new(optimize, mode, origin, waypoint1, waypoint2, waypoint3...)</pre>
where _origin_ and _waypoint*_ are strings of addresses or places that Google can find an address for (Example: "816 Meridian St., 37207") and _optimize_ is whether or not to optimize the route (string: true or false) and _mode_ is a string of the driving mode (driving/walking/bicycling).

Get drive time or distance of whole trip
<pre>
drive_time_in_minutes = directions.drive_time_in_minutes
distance_in_miles = directions.distance_in_km
</pre>
...
Rails 2.3
<pre>script/plugin install git://github.com/tomwolfe/google-directions-ruby.git</pre>

Rails 3.0
<pre>rails plugin install git://github.com/tomwolfe/google-directions-ruby.git</pre>
